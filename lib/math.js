...
const ChineseNum = {
	...
	toUpperCase: s => Array.from(s).map(c => ChineseNum.DIGITS.of(c)).filter(e => e).map(e => e.upperCase).join('')
};
...
(() => {
	...
	const 描述个位 = n => CnDigits[n].lowerCase;
	...
	const digitParse = num => {
		...
		.map(c => CnDigits.of(Number(c))?.lowerCase ?? c)//
		.join('');
		if (rt.startsWith('-')) {
	...
	const parseA = num => {
		...
		for (let i = sections.length - 1; i >= 0; i--) {// 分段处理（从高段到低段）
			...
			const unit = units[i].lowerCase;//todo：超过‘极’怎么处理？
		...
		if (rt.startsWith('一十')) {//去掉开头的“一十x”前面的“一”（如果要求使用“十x”而非“一十x”模式）
			rt = rt.substring(1);
		}
