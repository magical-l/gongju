...
(async () => {
	try {
		...
		const pickTargetFor = (skill, r, c) => {
			try {
				const list = (() => {
					...
					if (typeof skill.availableTargets === 'function') {
						return skill.availableTargets() || [];
					}
					if (typeof skill.availableTargets === 'object') {
						return skill.availableTargets || [];
					}
					if (typeof skill.getRawTargetPositions === 'function') {
						return skill.getRawTargetPositions() || [];
					}
					return [];
				})();
		...
		if (typeof QUnit !== 'undefined') {
			...
			if (QUnit.done) {
				QUnit.done(function(details) {
					...
					try {
						...
						p.id = 'test-summary';
						p.textContent
							= `tests completed: passed ${details.passed}, failed ${details.failed}, total ${details.total}`;
			...
			QUnit.test("红走兵(7,3)->(6,3)，黑走卒(4,1)->(5,1) 后应轮到红方", async function(assert) {
				...
				gaming.bulletin.notice('ui input',
					gaming.situation && gaming.situation.curPlayer || game && game.players && game.players[0]);
				...
				// 断言回合交替：应轮到红方
				await waitFor(
					() => gaming.situation && gaming.situation.curPlayer && gaming.situation.curPlayer.id === '红方玩家');
				const cur = gaming.situation && gaming.situation.curPlayer;
				assert.ok(cur && cur.id === '红方玩家', "当前玩家轮到红方");
			});

			QUnit.test("吃子后进驻 + 回合交替：红兵(7,3)->(6,3)->(5,3)，黑卒(4,1)->(5,1)->(6,1)，红兵(5,3)->(4,3) 吃卒",
				async function(assert) {
					...
					// 断言：回合交替 —— 吃子后应轮到黑方
					await waitFor(
						() => gaming.situation && gaming.situation.curPlayer && gaming.situation.curPlayer.id === '黑方玩家');
			...
			QUnit.test("红炮(3,2)->(3,3)；黑士(1,4)->(2,3)造屏；红炮(3,3)->(1,3) 吃黑相", async function(assert) {
				...
				const advSel = adv.skills.find(
													s => s.getRawTargetPositions && s.getRawTargetPositions().some(p => p.rowNum === 2 && p.colNum === 3))
												|| adv.skills[0];
				...
				const eatSkill = cannonAt33.skills.find(
					s => s.name && (s.name.includes('隔山') || s.name.includes('打牛') || s.name.includes('轮子')
													|| s.name.includes('攻击'))) || cannonAt33.skills[0];
				...
				// 等待：黑相被消灭，红炮进驻(1,3)
				await waitFor(() => {
					const u = bf.getUnitsAt(pt(1, 3))[0] || {};
					return u && (u.name === '砲' || u.name === '炮');
				});
	...
	function __helper(bf) {
		...
		return {
			clearAt(r, c) {
				const arr = bf.getUnitsAt(pt(r, c)) || [];
				arr.forEach(u => u && u.die && u.die());
			},
			place(name, sideId, r, c) {
				// 简化：从现有同名棋子克隆一枚到指定格；若无同名，取兵/卒占位
				const all = bf.getAllUnits ? bf.getAllUnits() : [];
				const src = all.find(u => u.name === name && u.owner && u.owner.id === sideId) || all.find(
					u => sideId === '红方玩家' ? u.name === '兵' : u.name === '卒');
				if (!src) {
					return;
				}
				// 如目标有子，先清空
				...
				} else if (src.moveTo) { try { src.moveTo(pt(r, c)); } catch (_) {} } else {
					// 若无API，直接尝试底层放置（若bf提供）
					if (bf.setUnitAt) {
						try { bf.setUnitAt(pt(r, c), src); } catch (_) {}
					}
				}
			},
		};
