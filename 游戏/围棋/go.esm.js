import {Player, TurnBasedGame} from './../turn-based-game.esm.js';
...
class GoGame extends BattlefieldBasedGaming {
				...
				for (let k = 0; k < n; k++) {
					...
					for (let i = changes.length - 1; i >= 0; i--) {
						...
						if (topic === 'battlefield destroyUnit end' && payload.unit && payload.place) {
							...
							if ('isAvailable' in payload.unit) {
								payload.unit.isAvailable = true;
							}
							continue;
						}
			...
			try {
				...
				for (let i = 0; i < list.length; i++) {
					...
					if (topic === 'buildUnit end' && payload.unit) {
						...
						if (place) {
							this.battlefield.addUnitToPosition(payload.unit, place);
						}
						// 同步手数为历史中该子的手数（保持对齐）
	...
	_extractLastTurnFromHistory() {
		...
		// 跳过非回合内的纯显示性事件（可选）
		while (end >= 0 && notices[end]?.topic === 'game over') {
			end--;
		}

		let start = end;
		while (start >= 0 && notices[start]?.topic !== 'playerTurn start') {
			start--;
		}
	...
	computeAreaScore() {
		...
		for (let r = 1; r <= n; r++) {
			for (let c = 1; c <= m; c++) {
				...
				if (this.unitAt(start) || visited.has(key(start))) {
					continue;
				}
...
class GoBoard extends Board {
	...
	starPoints() {
		...
		if (n === 19) {
			return this._star(19);
		}
		if (n === 13) {
			return this._star(13);
		}
		if (n === 9) {
			return this._star(9);
		}
...
class PlaceStoneCommand {
	...
	async execute() {
		...
		for (const nb of neighbors) {
			...
			if (!nbUnit || nbUnit.owner?.team?.id === this.player.team?.id) {
				continue;
			}
			const gk = groupKey(nb);
			if (visitedEnemyGroups.has(gk)) {
				continue;
			}
