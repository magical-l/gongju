import {
    TurnBasedGame, Player, Rule
} from './../turn-based-game.esm.js';
import {
    BattlefieldModule, Board, 棋盘点位, BattlefieldBasedGaming
...
class GobangGame extends BattlefieldBasedGaming {
    ...
    buildUnit(player, position, unitTypeCfg = {}) {
        ...
        console.log('GobangGame.buildUnit called for player:', player.id, 'at position:', position.toString(), 'Move #:', this._moveNumber);
        ...
        console.log('Calling this.battlefield.addUnitToPosition...');
        this.battlefield.addUnitToPosition(unit, position);
        console.log('Emitting buildUnit end event for unit:', unit);
...
class GobangPlayer extends Player {
    interpretInput(input) {
        console.log('interpretInput received:', input);
        // 输入应该是棋盘点位
        if (!(input instanceof 棋盘点位)) {
            console.log('interpretInput: Input is not 棋盘点位.');
            return null;
        }

        // 检查位置是否已有棋子
        const unitsAtTarget = this.gaming.battlefield.getUnitsAt(input);
        console.log('interpretInput: unitsAtTarget:', unitsAtTarget);
        if (unitsAtTarget.length > 0) {
            console.log('interpretInput: Position is occupied.');
            return null; // 位置不为空，无效操作
        }

        // 返回一个落子命令
        const command = new PlaceStoneCommand(this, input);
        console.log('interpretInput: Returning command:', command);
        return command;
    }
}
...
class PlaceStoneCommand {
    ...
    async execute() {
        console.log('Executing PlaceStoneCommand for player:', this.player.id, 'at position:', this.position.toString());
        const gaming = this.player.gaming;
        gaming.startChangeCollection();

        // 创建新棋子
        console.log('Calling gaming.buildUnit...');
        gaming.buildUnit(this.player, this.position);

        const changes = gaming.stopChangeCollection();
        console.log('PlaceStoneCommand: Changes collected:', changes);
        ...
        for (const [dr, dc] of directions) {
            ...
            for (let sign = -1; sign <= 1; sign += 2) {
                ...
                for (let i = 1; i < 5; i++) {
                    ...
                    if (!battlefield.isValidPosition(pos)) break;
