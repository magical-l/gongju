...
function startTurnTimer() {
	stopTurnTimer();
	if (paused || !gameState || gameState.gameOver || gameState.gameWin) {
		return;
	}
...
function handleDirection(newDir) {
	...
	if (!canSetDirection(gameState, newDir)) {
		return;
	}
...
function loadSettingsFromStorage() {
	try {
		...
		if (Number(o.turnIntervalMs) >= 50) {
			s.turnIntervalMs = Number(o.turnIntervalMs);
		}
		if (Number(o.initialLength) >= 1) {
			s.initialLength = Number(o.initialLength);
		}
		if (Number(o.foodCount) >= 1) {
			s.foodCount = Number(o.foodCount);
		}
...
function applyBoardSettings(obj) {
	...
	if (needRestart) {
		...
		if (opts.turnIntervalMs != null && !paused) {
			startTurnTimer();
		}
	}
...
function doInit() {
	...
	if (useRestored) {
		...
		if (restored) {
			if (opts.targetNumber === undefined && restored.targetNumber !== undefined) {
				opts.targetNumber
					= restored.targetNumber;
			}
			if (opts.turnIntervalMs == null && restored.turnIntervalMs != null) {
				opts.turnIntervalMs = restored.turnIntervalMs;
			}
			if (opts.initialLength == null && restored.initialLength != null) {
				opts.initialLength = restored.initialLength;
			}
			if (opts.foodCount == null && restored.foodCount != null) {
				opts.foodCount = restored.foodCount;
			}
		}
...
(function() {
	...
	document.addEventListener('keydown', function(e) {
		...
		if (e.key === ' ') {
			...
			var isEditable = active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA' || active.tagName
