...
序列别名（写 符号富化数据.js）：
  ...
  别名一律**落条目级**（= 全局别名）、**不进名字层**、也不当显示名。
  ⚠️ 组里只放**语境名**（`groups[标签].name`）——「有组 = 有语境名」是硬规则，
     别名挂进组里只会在别的标签下看不见它（v1.39.2 把 3804 条组内别名全提到了条目级）。
...
def seq_tag(cps, roots):
    ...
    ⚠️ **当前无调用方**：别名不再挂"最深语义标签"（那会建出只装别名的无名组），
    只挂有语境名的组、否则落条目级。留着是因为它是"该序列属哪个标签"的通用判据，
    将来若要按标签做点别的还用得上——但**别拿它去建组键**。
    """
    target = list(cps)
    best = [-1, None]
        ...
        if zh and 0x27A1 not in cps:
            ...
            if not any(is_skin(c) for c in cps):
                ...
                if al:
                    ...
                    # 组数 3533 → 2278、无名组归零），别再把别名挂回组里。
                    slot.setdefault(None, {})['alias'] = al
            if not slot:
                seq_meta.pop(ch)
        for path in paths:
