...
if (!logic || !constants) {
  ...
  function runSlideAnimation(slides, finalState, stateBeforeMove, newTileIndex) {
    if (slideAnimationActive || !slides || !slides.length) {
      ...
      if (view && view.commitState) view.commitState(gameState);
      if (view && view.setNewTileIndex) view.setNewTileIndex(newTileIndex);
      return;
    }
  ...
  function clearSlideAnimation() {
    slideAnimationActive = false;
    if (view && view.clearSlideAnimation) view.clearSlideAnimation();
  }

  function handleMove(direction) {
    if (view && view.clearNewTileIndex) view.clearNewTileIndex();
    ...
    if (result.slides && result.slides.length > 0) {
      ...
      if (view && view.commitState) view.commitState(gameState);
      if (view && view.setNewTileIndex) view.setNewTileIndex(result.newTileIndex);
    }
  }
  ...
  function doInit() {
    ...
    if (view && view.setQuickSettings) {
      view.setQuickSettings({ showNewTileMarker: gameState.showNewTileMarker, newTileOnMidStop: gameState.newTileOnMidStop });
    }
    if (view && view.syncToolbarSettings) view.syncToolbarSettings();
  }
