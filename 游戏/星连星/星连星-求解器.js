...
function findHamiltonian(start, end, cellsSet) {
  ...
  if (!cellsSet.has(startK) || !cellsSet.has(endK)) return null;
  ...
  function dfs(path, used) {
    ...
    for (const [nr, nc] of adj(r, c)) {
      const k = key(nr, nc);
      if (!cellsSet.has(k) || used.has(k)) continue;
      used.add(k);
      path.push([nr, nc]);
...
function* bluePaths() {
  ...
  used.add(key(1,4)); used.add(key(1,5)); // 绿

  function* dfs(path) {
    const [r, c] = path[path.length - 1];
    if (path.length === BLUE_LEN) {
      if (r === BLUE_END[0] && c === BLUE_END[1]) yield path.map(x => [...x]);
      return;
    }
    for (const [nr, nc] of adj(r, c)) {
      const k = key(nr, nc);
...
for (const bluePath of bluePaths()) {
  ...
  for (let r = 1; r <= R; r++)
    for (let c = 1; c <= C; c++) {
      const k = key(r, c);
      if (GREEN_SET.has(k) || bs.has(k)) continue;
      rc.add(k);
    }
  if (rc.size !== 16) continue;
...
if (!found) console.log('结论: 该星位布局无解。');
